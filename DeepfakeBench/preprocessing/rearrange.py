...
def generate_dataset_file(dataset_name, dataset_root_path, output_file_path, compression_level='c23', perturbation = 'end_to_end'):
    ...
    ## FFIW dataset
        ...
        dataset_path = os.path.join(dataset_root_path, dataset_name)
        dataset_dict[dataset_name] = {
            'FFIW_Real': {'train': {}, 'test': {}, 'val': {}},
            'FFIW_Fake': {'train': {}, 'test': {}, 'val': {}}
        }
        for split in ['train', 'val']:
            # Real: source/<split>/frames
            src_dir = os.path.join(dataset_path, 'source', split, 'frames')
            if os.path.isdir(src_dir):
                for video_d in os.scandir(src_dir):
                    if video_d.is_dir():
                        frame_paths = [os.path.join(video_d, f.name) for f in os.scandir(video_d)]
                        dataset_dict[dataset_name]['FFIW_Real'][split][video_d.name] = {
                            'label': 'FFIW_Real', 'frames': frame_paths
                        }
            # Fake: target/<split>/frames
            tgt_dir = os.path.join(dataset_path, 'target', split, 'frames')
            if os.path.isdir(tgt_dir):
                for video_d in os.scandir(tgt_dir):
                    if video_d.is_dir():
                        frame_paths = [os.path.join(video_d, f.name) for f in os.scandir(video_d)]
                        dataset_dict[dataset_name]['FFIW_Fake'][split][video_d.name] = {
                            'label': 'FFIW_Fake', 'frames': frame_paths
                        }
        # val -> test
        for label in ['FFIW_Real', 'FFIW_Fake']:
            dataset_dict[dataset_name][label]['test'] = dataset_dict[dataset_name][label]['val']

    ## WDF (WildDeepfake) dataset
    elif dataset_name == 'WDF':
        dataset_path = os.path.join(dataset_root_path, dataset_name)
        dataset_dict[dataset_name] = {
            'WDF_Real': {'train': {}, 'test': {}},
            'WDF_Fake': {'train': {}, 'test': {}}
        }
        splits = [('real_train', 'WDF_Real', 'train'),
                  ('real_test',  'WDF_Real', 'test'),
                  ('fake_train', 'WDF_Fake', 'train'),
                  ('fake_test',  'WDF_Fake', 'test')]
        for src_dir, label, split in splits:
            frames_root = os.path.join(dataset_path, src_dir, 'frames')
            if not os.path.isdir(frames_root):
                continue
            for video_d in os.scandir(frames_root):
                if video_d.is_dir():
                    frame_paths = sorted(
                        os.path.join(video_d, f.name) for f in os.scandir(video_d)
                    )
                    dataset_dict[dataset_name][label][split][video_d.name] = {
